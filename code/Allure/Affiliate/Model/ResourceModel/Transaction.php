...
class Transaction extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
	...
	{
		$this->_init('allure_affiliate_transaction', 'transaction_id');
	}

	/**
	 * before save callback
	 *
	 * @param \Magento\Framework\Model\AbstractModel|\Allure\Affiliate\Model\Transaction $object
	 * @return $this
	 */
	...
	public function updateAmountUsed($transaction, $totalAmount = null)
	{
		...
		$binds = ['customer_id' => (int)$transaction->getCustomerId(), 'status' => \Allure\Affiliate\Model\Transaction\Status::STATUS_COMPLETED];

		if ($transaction->getIsUpdateHoldingTransaction()) {
			$select->where('order_id = :order_id');
			$binds['order_id'] = (int)$transaction->getOrderId();
			$binds['status']   = \Allure\Affiliate\Model\Transaction\Status::STATUS_HOLD;
		}
		...
		if (count($usedIds)) {
			$this->getConnection()->update(
				$this->getMainTable(),
				['amount_used' => new \Zend_Db_Expr('amount'), 'status' => \Allure\Affiliate\Model\Transaction\Status::STATUS_COMPLETED],
