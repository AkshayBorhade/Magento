...
namespace Allure\Affiliate\Helper\Calculation;

use Allure\Affiliate\Helper\Calculation;
        ...
        foreach ($this->getAvailableCampaign($account) as $campaign) {
            ...
            foreach ($commissions as $key => $tier) {
                ...
                switch ($tier['type' . $fieldSubfix]) {
                    case \Allure\Affiliate\Block\Adminhtml\Campaign\Edit\Tab\Commissions\Arraycommission::TYPE_FIXED:
                        $commissionResult[$tierId] += $this->priceCurrency->round($commission);
                        break;
                    case \Allure\Affiliate\Block\Adminhtml\Campaign\Edit\Tab\Commissions\Arraycommission::TYPE_SALE_PERCENT:
