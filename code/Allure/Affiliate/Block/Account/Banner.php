...
class Banner extends \Allure\Affiliate\Block\Account
{
	...
	public function getAvailableBanners()
	{
		...
		$banner           = $this->objectManager->create('Allure\Affiliate\Model\Banner');
		$bannerCollection = $banner->getCollection()
			->addFieldToFilter('campaign_id', ['in' => $campaigns])
			->addFieldToFilter('status', \Allure\Affiliate\Model\Banner\Status::ENABLED);
	...
	public function getLink($banner)
	{
		...
		return $this->_affiliateHelper->getSharingUrl($url, ['source' => 'banner', 'key' => $banner->getId()], \Allure\Affiliate\Model\Config\Source\Urltype::TYPE_PARAM);
	}
